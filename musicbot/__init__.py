...
class Yikes:
    ...
    def load_module(self, name):
        ...
        raise HelpfulError(
            "リクエストをインポートしようとしている、またはリクエストを使用するモジュールをインポートしようとしている。  "
            "リクエスト（またはリクエストを使用するモジュール）は、このコードでは使用しないでください。 "
            "リクエストがこのコードに適さない理由については、%sを参照してください。"
            % "[https://discordpy.readthedocs.io/en/latest/faq.html#what-does-blocking-mean]",

            "代わりにaiohttp使用し、要求を使用しないでください。APIは、セッションオブジェクトを使用する場合のリクエストに"
            "非常に似ています。 [http://aiohttp.readthedocs.io/en/stable/]  If "
